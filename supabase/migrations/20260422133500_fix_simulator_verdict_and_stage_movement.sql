drop function if exists public.append_simulation_exchange(text, text, text, text, jsonb, text, text);
...
begin
  ...
  values (
    ...
    trim(ai_message),
    ai_model,
    case when coalesce(ai_should_close, false) then 'closing_note' else 'simulator_follow_up' end,
    normalized_sentiment,
...
revoke all on function public.append_simulation_exchange(text, text, text, text, jsonb, text, text, text, text, boolean) from public;
grant execute on function public.append_simulation_exchange(text, text, text, text, jsonb, text, text, text, text, boolean) to anon, authenticated;
